...
<body>
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<form:form class="form-horizontal" role="form" id="formSubmit"
							...
							<div class="form-group">
								<label class="col-sm-3 control-label no-padding-right"
									for="form-field-1">Link ảnh đại diện</label>
								<div class="col-sm-9">
									<form:input path="thumbnail" class="col-xs-10 col-sm-5" />
								</div>
							</div>
							<div class="form-group">
							...
							<div class="clearfix form-actions">
								<div class="col-md-offset-3 col-md-9">
									...
									&nbsp; &nbsp; &nbsp; <a class="btn" href='${home}'><i
										class="ace-icon fa fa-undo bigger-110"></i> Hủy </a>
								</div>
							</div>
						</form:form>
					</div>
	...
	<script>
		...
		$('#btnAddOrUpdateNew').click(function(e) {
			e.preventDefault();
			var data = {};
			//data["thumbnail"] = $('input[type=file]').val().split('\\').pop();
			var formData = $('#formSubmit').serializeArray();
			$.each(formData, function(i, v) {
				if (v.name == "content") {
					data["" + v.name + ""] = content.getData();
				} else {
					data["" + v.name + ""] = v.value;
				}
			});
			...
			$
					.ajax({
						...
						error : function(error) {
							window.location.href = "${newURL}?page=1&limit5&message=error_system";
						}
					});
		}
